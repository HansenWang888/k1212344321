...
#import "DDDatabaseUtil.h"
#define IMAGE_DOWNLOAD_FILE @"../../images/i_noimg.png"
#define AUDIO_DOWNLOAD_FILE @"disable.png"
#define IMAGE_DISABLE       @"../../images/i_noimg.png"
#define AUDIO_DISABLE       @"disable.png"

#define AUDIO_ENABLE        @"enable.png"
...
@implementation DDMessageModule
{
    ...
    NSMutableIndexSet *  _sendingMessages;
}
...
- (id)init
{
    ...
    if (self)
    {
        ...
        _sendingMessages = [NSMutableIndexSet new];
...
-(void)addSendingMessage:(NSInteger) msgid{
    [_sendingMessages addIndex:msgid];
}
-(BOOL)isInSendingQueue:(NSInteger) msgid{
    return [_sendingMessages containsIndex:msgid];
}
...
-(void)removeFromUnreadMessageButNotSendRead:(NSString*)sessionID
{
    ...
    WDULog(@" remove message %d--->,%@ id is ",[messages count],sessionID);
    if ([messages count]> 0)
    {
...
- (void)p_registerReceiveMessageAPI
{
    ...
    [receiveMessageAPI registerAPIInAPIScheduleReceiveData:^(id arrayobj, NSError *error) {
        ...
        [DDNotificationHelp postNotification:DDNotificationReceiveMessage userInfo:nil object:arrayobj];
    }];
