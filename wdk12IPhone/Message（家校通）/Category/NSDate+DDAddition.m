...
- (NSString*)transformToFuzzyDate
{
    ...
    {
        return IMLocalizedString(@"刚刚", nil);
    }
    else if (selfTnterval < 60 * 60)
    {
        NSString* dateString = [NSString stringWithFormat:@"%lu%@",selfTnterval / 60, IMLocalizedString(@"分钟前", nil)];
        return dateString;
    }
    else if (selfTnterval < 24 * 60 * 60 && nowDateComponents.day == selfDateComponents.day)
    {
        NSString* dateString = [NSString stringWithFormat:@"%lu%@",selfTnterval / (60 * 60), IMLocalizedString(@"小时前", nil)];
        return dateString;
    }
    else if (timeInterval < 48 * 60 * 60)
    {
        return IMLocalizedString(@"昨天", nil);
    }
    else if (nowDateComponents.week == selfDateComponents.week)
    {
        NSArray* weekdays = @[@"temp",@"Sunday",@"Monday",@"Tuesday",@"Wednesday",@"Thursday",@"Friday",@"Saturday"];
    ...
    if (timeInterval < 24 * 60 * 60)
    {
        if (nowDateComponents.day == selfDateComponents.day) {
            ...
            if (selfDateComponents.hour <= 4) {
                dateString = [NSString stringWithFormat:@"%@ %@",IMLocalizedString(@"凌晨", nil), lastComponents];
            }else if (selfDateComponents.hour <= 12){
                dateString = [NSString stringWithFormat:@"%@ %@",IMLocalizedString(@"上午", nil), lastComponents];
            }else if (selfDateComponents.hour <= 18){
                dateString = [NSString stringWithFormat:@"%@ %@",IMLocalizedString(@"下午", nil), lastComponents];
            }else{
                dateString = [NSString stringWithFormat:@"%@ %@",IMLocalizedString(@"晚上", nil), lastComponents];
            }
        ...
        {
            //昨天
            dateString = [NSString stringWithFormat:@"%@ %@",IMLocalizedString(@"昨天", nil), lastComponents];
        }
    }
    else if (nowDateComponents.week == selfDateComponents.week)
    {
        ...
        dateString = [NSString stringWithFormat:@"%@%@",IMLocalizedString(weekdayString, nil),lastComponents];
    }
    else
    {
        NSDateFormatter* dateFormatter = [[NSDateFormatter alloc] init];
