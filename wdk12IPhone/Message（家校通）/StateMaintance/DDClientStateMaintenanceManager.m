...
- (void)dealloc
{
    WDULog(@"DDClientStateMaintenanceManager release");
...
- (void)p_startCheckServerHeartBeat
{
    ...
    {
        WDULog(@"begin maintenance _serverHeartBeatTimer");
...
- (void)p_startRelogin
{
    ...
    if (!_reloginTimer)
    {
        ...
        [_reloginTimer fire];
    //    [[NSRunLoop currentRunLoop] addTimer:_reloginTimer forMode:NSDefaultRunLoopMode];
    }
}
...
- (void)p_onReloginTimer:(NSTimer*)timer
{
    ...
    if (time >= _reloginInterval)
    {
        ...
        [[LoginModule instance] reloginSuccess:^{
            ...
            WDULog(@"relogin success");
            } failure:^(NSString *error) {
            WDULog(@"relogin failure:%@",error);
                NSLog(@"--------error-----------:%@",error);
                if ([error isEqualToString:@"未登录"]) {
                 ...
                 //   NSLog(@"_reloginInterval;;;;;;;;;%ld",_reloginInterval);
                }
