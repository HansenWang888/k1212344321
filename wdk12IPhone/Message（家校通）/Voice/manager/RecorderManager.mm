    ...
    if ( ! mAQRecorder->IsRunning()) {
        ...
        if ( ! recordingWillBegin) {
            if ([self.delegate respondsToSelector:@selector(recordingFailed:)]) {
                [self.delegate recordingFailed:IMLocalizedString(@"程序错误，无法继续录音，请重启程序试试", nil)];
            }
            return;
        }
    }
...
- (NSTimeInterval)recordedTimeInterval {
  //  [testCSS CreateFunc];
    ...
    WDULog(@"record start time %@______ record stop time %@",dateStartRecording,dateStopRecording);
