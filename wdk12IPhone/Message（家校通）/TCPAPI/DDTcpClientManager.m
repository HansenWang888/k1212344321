...
{
    WDULog(@"mogujie mtalk client :connect ipAdr:%@ port:%ld",ipAdr,(long)port);
    cDataLen = 0;
...
-(void)disconnect
{
    WDULog(@"MTalk Client:disconnect");
       ...
       //     DDLog(@"WRITE - Written directly to outStream len:%li", (long)len);
        ...
        if (_lastSendBuffer) {
            ...
            if (lastSendBufferLength<1024) {
                WDULog(@"WRITE - Have a buffer with enough space, appending data to it");
                [_lastSendBuffer appendData:data];
                return;
            }
        }
        WDULog(@"WRITE - Creating a new buffer");
        _lastSendBuffer = [DDSendBuffer dataWithNSData:data];
        [_sendBuffers addObject:_lastSendBuffer];
        
    }
    @catch (NSException *exception) {
        WDULog(@" ***** NSException:%@ in writeToSocket of MGJMTalkClient *****",exception);
    }
    @finally {
        [_sendLock unlock];
    }
...
- (void)p_handleConntectOpenCompletedStream:(NSStream *)aStream
{
    WDULog(@"handleConntectOpenCompleted");
...
- (void)p_handleEventHasSpaceAvailableStream:(NSStream *)aStream
{
   ...
   //     DDLog(@"WRITE - Written directly to outStream len:%lid", (long)len);
    ...
    @catch (NSException *exception) {
        WDULog(@" ***** NSException in MGJMTalkCleint :%@  ******* ",exception);
    }
    @finally {
        [_sendLock unlock];
    }
...
- (void)p_handleEventErrorOccurredStream:(NSStream *)aStream
{
    WDULog(@"handle eventErrorOccurred");
    [self disconnect];
...
- (void)p_handleEventEndEncounteredStream:(NSStream *)aStream
{
    WDULog(@"handle eventEndEncountered");
    ...
    [DDClientState shareInstance].userState = DDUserOffLine;
//    
//    NSNotification* losconnect = [[NSNotification alloc]initWithName:@"EventEndEncounteredStream" object:nil userInfo:nil];
//    
//    [[NSNotificationQueue defaultQueue]enqueueNotification:losconnect postingStyle:NSPostASAP];
        ...
        if (len > 0) {
            ...
            while ([_receiveBuffer length] >= 16) {
                ...
                if (pduLen > (uint32_t)[_receiveBuffer length]) {
                    WDULog(@"not enough data received");
                    break;
                }
...
//                WDULog(@"receive a packet serviceId=%d, commandId=%d", tcpHeader.serviceId, tcpHeader.commandId);
