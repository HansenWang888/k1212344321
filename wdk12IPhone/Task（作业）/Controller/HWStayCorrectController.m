...
@interface HWStayCorrectController ()<UICollectionViewDelegate, UICollectionViewDataSource>
...
@implementation HWStayCorrectController
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.taskModel.zylx isEqualToString:@"1"]) { // 非在线作业
        ...
        vc.isSubmit = true;
        vc.correctFinish = ^(StudentModel *model) {
            if (![weakSelf.alreadyVC.data containsObject:model]) {
                weakSelf.alreadyVC.notOnlineData = weakSelf.notOnlineData;
                [weakSelf.data removeObject:model];
                [weakSelf.alreadyVC.data addObject:model];
                [weakSelf.collectionView reloadData];
                [weakSelf.alreadyVC.collectionView reloadData];
                [(HWTaskCheckViewController *)weakSelf.parentViewController resetSubmitStatusCount];
            }
        };
        ...
        vc.isFeedback = false;
        vc.taskModel = self.taskModel;
        NSArray *array = self.onlineData;
        for (NSDictionary *item in array) {
            if ([item[@"xsID"] isEqualToString:st.id]) {
                vc.studentAnswer = item;
            }
        }
        vc.student = st;
        vc.correctFinish = ^(StudentModel *model) {
        ...
        [self.parentViewController.navigationController pushViewController:vc animated:true];
    } else if ([self.taskModel.zylx isEqualToString:@""]) {
        HWPreviewViewController *vc = [HWPreviewViewController new];
        vc.isSubmit = true;
        vc.taskId = self.taskModel.zyID;
        vc.studentId = [(StudentModel *)self.data[indexPath.row] id];
        [self.parentViewController.navigationController pushViewController:vc animated:true];
