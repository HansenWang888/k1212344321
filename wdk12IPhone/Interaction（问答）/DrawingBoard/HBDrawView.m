...
- (HBDrawingBoard *)drawBoard
{
    if (!_drawBoard) {
        ...
        [_drawBoard getDrawedImage:^(UIImage *drawedImage) {
            if (_delegate && [_delegate respondsToSelector:@selector(getFinishedImage:)]) {
                [_delegate getFinishedImage:drawedImage];
            }
        }];
    }
    return _drawBoard;
